...
class QualityMetrics:
    def __init__(self) -> None:
        if telemetry.enable_metrics and Gauge is not None:
            self.precision = Gauge('ai_core_quality_precision', 'Precision@k', ['tenant', 'suite'])
            self.recall = Gauge('ai_core_quality_recall', 'Recall@k', ['tenant', 'suite'])
    ...
    def set_eval(self, tenant: str, suite: str, p: float, r: float, f1: float) -> None:
        try:
            if self.precision: self.precision.labels(tenant=tenant, suite=suite).set(float(p))
            if self.recall: self.recall.labels(tenant=tenant, suite=suite).set(float(r))
            if self.f1: self.f1.labels(tenant=tenant, suite=suite).set(float(f1))
        except Exception:
            pass

    def set_reranker_acc(self, tenant: str, acc: float) -> None:
        try:
            if self.reranker_acc: self.reranker_acc.labels(tenant=tenant).set(float(acc))
        except Exception:
            pass
