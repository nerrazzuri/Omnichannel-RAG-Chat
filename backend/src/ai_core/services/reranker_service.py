...
class RerankerCache:
    ...
    def get_cross_encoder_scores(self, query: str, docs: List[str]) -> Optional[List[float]]:
    ...
    def set_cross_encoder_scores(self, query: str, docs: List[str], scores: List[float]):
...
class AdvancedReranker:
    ...
    async def _performance_context(self, operation: str):
        ...
        try:
            ...
            logger.info(f"Reranking operation '{operation}' completed in {duration:.3f}s")
    ...
    def multi_stage_reranking(self, 
        ...
        logger.info(f"Starting reranking for {len(documents)} documents, query: {query[:100]}...")
        ...
        try:
            ...
            if self.ltr_available and len(top_docs) > 5:
                final_results = self._ltr_rerank(query, top_docs, top_bi_scores, cross_scores, top_indices)
                method_used = "ltr"
            else:
            ...
            logger.info(f"Reranking completed: {len(result.documents)} results in {processing_time:.3f}s using {method_used}")
            return result
            ...
            return RerankingResult(
                ...
                scores=bi_encoder_scores[:top_k] if bi_encoder_scores else [1.0] * min(top_k, len(documents)),
                original_indices=original_indices[:top_k],
                processing_time=processing_time,
                method_used="fallback"
            )
    
    async def async_multi_stage_reranking(self, 
                                        query: str, 
                                        documents: List[str], 
                                        bi_encoder_scores: Optional[List[float]] = None,
                                        top_k: int = 10) -> RerankingResult:
        ...
        if not self.enable_async:
            return self.multi_stage_reranking(query, documents, bi_encoder_scores, top_k)
    ...
    def _weighted_fusion(self, bi_scores: List[float], cross_scores: List[float]) -> List[float]:
        ...
        # Weighted fusion
        fused_scores = [bi_weight * b + cross_weight * c for b, c in zip(bi_norm, cross_norm)]
        
        return fused_scores
    
    def _ltr_rerank(self, 
                   query: str, 
                   documents: List[str],
                   bi_scores: List[float], 
                   cross_scores: List[float],
                   original_indices: List[int]) -> List[Tuple[str, float, int]]:
        """Learning-to-Rank reranking."""
        try:
            # Extract features
            features = self._extract_ranking_features(query, documents, bi_scores, cross_scores)
    ...
    def _extract_ranking_features(self, 
                                ...
                                documents: List[str],
                                bi_scores: List[float],
                                cross_scores: List[float]) -> List[RankingFeatures]:
        """Extract comprehensive ranking features for LTR."""
        features = []
        query_tokens = set(query.lower().split())
        
        for i, (doc, bi_score, cross_score) in enumerate(zip(documents, bi_scores, cross_scores)):
            ...
            feature = RankingFeatures(
                ...
                query_doc_overlap=len(query_tokens & doc_tokens) / max(1, len(query_tokens | doc_tokens)),
                doc_position_rank=i,
    ...
    def train_ltr_model(self, training_data: List[Dict[str, Any]], validation_data: List[Dict[str, Any]] = None):
        """
        ...
        try:
            ...
            if validation_data:
                val_features, val_labels, val_groups = self._prepare_validation_data(validation_data)
                val_X_scaled = self.feature_scaler.transform(val_features)
                
                self.ltr_model.fit(
                    X_scaled, y, group=groups,
    ...
    def _prepare_validation_data(self, validation_data: List[Dict[str, Any]]) -> Tuple[np.ndarray, np.ndarray, List[int]]:
        ...
        if self.ltr_model is not None:
            try:
                ...
                model_data = {
                    ...
                    'feature_names': [
                        'bi_encoder_score', 'cross_encoder_score', 'bm25_score',
                        'query_length', 'doc_length', 'entity_overlap',
                        'semantic_similarity', 'query_doc_overlap', 'doc_position_rank',
                        'tfidf_similarity'
                    ]
                }
