...
class HybridRetriever:
    ...
    def dense_search(self, query: str, top_k: int = 5) -> List[int]:
        # Minimal dense scoring approximation: length similarity to query
        q_len = len(query)
        scored = [(i, 1.0 / (1.0 + abs(len(doc) - q_len))) for i, doc in enumerate(self.corpus)]
    ...
    def retrieve(self, query: str, top_k: int = 5) -> List[str]:
        if not self.corpus:
            return []
        # Increase weight for exact name matches to improve row retrieval
