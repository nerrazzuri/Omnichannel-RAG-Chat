    ...
    def run(self, goal: str, tenant_id: str, claims: Dict[str, Any], db) -> Dict[str, Any]:
        ...
        for step in plan:
            ...
            if decision.requires_approval:
                steps_out.append({"tool_id": tool.spec.tool_id, "status": "pending_approval", "approval_id": decision.approval_id})
                try:
                    write_audit(db, tenant_id, claims.get("user_id"), "agent.tool.approval.request", tool.spec.tool_id, redact(str(params)), "", True, int((time.time()-t0)*1000), category="agent", auth_type=claims.get("auth_type"))
                except Exception:
                    pass
                continue
            if not decision.allowed:
                agent_tool_metrics.inc_denial(tenant_id, tool.spec.tool_id)
                steps_out.append({"tool_id": tool.spec.tool_id, "status": "denied", "reason": decision.reason})
                try:
                    write_audit(db, tenant_id, claims.get("user_id"), "agent.tool.deny", tool.spec.tool_id, redact(str(params)), decision.reason or "", False, int((time.time()-t0)*1000), category="agent", auth_type=claims.get("auth_type"))
                except Exception:
                    pass
                continue
            # Execute tool
            try:
                ...
                res = tool.execute(tenant_id=tenant_id, api_key_id=claims.get("api_key_id"), payload=params)
                steps_out.append({"tool_id": tool.spec.tool_id, "status": res.get("status"), "output_summary": {k: v for k, v in res.items() if k != "status"}})
                try:
                    write_audit(db, tenant_id, claims.get("user_id"), "agent.tool.call", tool.spec.tool_id, redact(str(params)), redact(str(res))[:500], True, int((time.time()-t0)*1000), category="agent", auth_type=claims.get("auth_type"))
                except Exception:
                    pass
            except Exception as e:
                agent_metrics.inc_failure(name, tenant_id)
                logging.getLogger(__name__).exception("[agent.execute] error", extra={"tenant_id": tenant_id, "action": action, "agent": name})
                steps_out.append({"tool_id": tool.spec.tool_id, "status": "error", "error": str(e)})
        dur = time.time() - t0
