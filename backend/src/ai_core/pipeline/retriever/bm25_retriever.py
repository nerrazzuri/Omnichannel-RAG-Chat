...
class BM25Retriever:
    ...
    def build_corpus(
        ...
        tenant_id: str,
        limit: int = 2000,
    ) -> Tuple[List[str], List[str], Dict[str, str], Dict[str, Dict[str, str]], Dict[str, str], Dict[str, Dict[str, Any]]]:
        """Return (corpus_texts, idx_to_id, id_to_content, content_to_row, sig_to_id, text_to_docmeta)."""
        # Cache lookup
        now = time.time()
        with self._lock:
            entry = self._cache.get(tenant_id)
            if entry and (now - float(entry.get("ts", 0))) < max(5, int(getattr(retrieval, 'bm25_cache_ttl_s', 600))):
        ...
        for kc, _doc, _kb in tenant_pairs:
            ...
            if isinstance(c, str) and c.strip() and kid:
                ...
                try:
                    ...
                    text_to_docmeta[c] = {"document_id": str(getattr(_doc, 'id', '')), "title": str(title) if title else "", "source_url": str(src) if src else ""}
                except Exception:
                    pass
                try:
        ...
        # Cache new corpus
        with self._lock:
            self._cache[tenant_id] = {"ts": now, "corpus": corpus_texts, "text_meta": text_to_docmeta}
        return corpus_texts, idx_to_id, id_to_content, content_to_row, sig_to_id, text_to_docmeta
