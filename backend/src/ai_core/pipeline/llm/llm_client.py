...
class LLMClient:
    ...
    def generate(self, query: str, contexts: List[str], intent: str = "lookup", result_hint: str | None = None, tenant_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if tenant_id:
            ...
            if cached:
                ...
                return {"text": cached.get("text", ""), "used": cached.get("used", False)}
            else:
        ...
        try:
            ...
            # Compute cost
            m = self.model
            in_rate = float(cost_cfg.model_in_usd_per_1k.get(m, cost_cfg.model_in_usd_per_1k.get("default", 0.003)))
            out_rate = float(cost_cfg.model_out_usd_per_1k.get(m, cost_cfg.model_out_usd_per_1k.get("default", 0.006)))
            usd = (prompt_tokens/1000.0)*in_rate + (completion_tokens/1000.0)*out_rate
            if tenant_id:
                cost_metrics.record_tokens(tenant_id, m, "chat", prompt_tokens, completion_tokens, usd)
                rolling_cost.add(tenant_id, m, "chat", prompt_tokens, completion_tokens, usd)
                # Cache store
                try:
                    generation_cache.set(tenant_id, self.model, prompt, {"text": text, "used": True})
                except Exception:
                    pass
            try:
            ...
            try:
                retry_queue.enqueue("llm_generate", tenant_id="global", payload={"intent": intent, "has_ctx": bool(contexts)}, last_error=str(e))
            except Exception:
                pass
            return {"text": text, "used": False}
