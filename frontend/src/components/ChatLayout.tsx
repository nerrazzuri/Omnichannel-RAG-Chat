    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      <header className="sticky top-0 z-20 backdrop-blur bg-white/70 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1
              className="text-lg font-semibold text-gray-900 truncate max-w-[60vw]"
              title={title}
            >
              {title}
            </h1>
        ...
        {(planType || usage) && (
          <div className="bg-blue-50 border-t border-blue-100 text-blue-800 text-xs py-2 px-4">
            {planType && (
              <span className="mr-3">
                Plan: <span className="font-medium uppercase">{planType}</span>
              </span>
            )}
            {usage && typeof usage.tokens_quota !== "undefined" && (
              <span className="mr-3">
                Tokens: {usage.tokens_used}/{usage.tokens_quota}
              </span>
            )}
            {usage?.docs_count !== undefined && (
              <span className="mr-3">Docs: {usage.docs_count}</span>
            )}
            {planType === "free" && (
              <span className="ml-2">
                Consider upgrading to <span className="font-medium">Pro</span>{" "}
                for higher limits.
              </span>
            )}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-12 gap-4 py-4">
          {sidebar ? (
            <aside className="hidden lg:block col-span-3">{sidebar}</aside>
          ) : null}
          <main
            className={sidebar ? "col-span-12 lg:col-span-9" : "col-span-12"}
          >
            {children}
          </main>
        </div>
      ...
      <div className="fixed left-0 right-0 bottom-0 bg-gradient-to-t from-white to-white/70 backdrop-blur border-t border-gray-200">
        <div className="max-w-4xl mx-auto px-4 py-4">
          ...
          <div className="mt-1 text-[11px] text-gray-500">
            Answers are grounded in your uploaded knowledge. Sources are listed
            when available.
          </div>
        </div>
      </div>
