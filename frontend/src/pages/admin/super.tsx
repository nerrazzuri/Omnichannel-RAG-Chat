...
export default function SuperAdmin() {
  ...
  const onCreateTenant = async () => {
    ...
    setMsg("");
    try {
      const res = await createTenant(cfg, {
        name: newTenantName,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { getGatewayHealth, getAiCoreHealth, getComplianceSummary } =
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            ...
            <input
              ...
              placeholder="paste admin JWT/API key here"
            />
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
            ...
            <div>
              <button
                ...
                disabled={creatingTenant || !newTenantName || !newTenantDomain}
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
              ...
              <div>
                ...
                <div>{tenantInfo.created_at}</div>
              </div>
              <div>
                <div className="text-gray-500">Updated</div>
                <div>{tenantInfo.updated_at}</div>
              </div>
              <div>
              ...
              <div className="border rounded p-3">
                ...
                <div className="mt-1">
                  ...
                  <div>
                    ...
                    {(
                      Math.round(
                        ((costSnapshot?.cost_cents ?? 0) / 100) * 100,
                      ) / 100
                    ).toFixed(2)}
                  </div>
                </div>
                ...
                {grafana && (
                  <div className="mt-2">
                    <a
                      ...
                      href={`${grafana}/d/Release_Readiness/Release_Readiness?var-tenant=${encodeURIComponent(tenantId)}`}
                      target="_blank"
                      rel="noreferrer"
                    >
                      Grafana: Release Readiness
                    </a>
                  </div>
                )}
              </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6">
          ...
          <div className="space-y-2">
            {retention.map((p, idx) => (
              <div
                key={idx}
                className="border rounded p-2 flex flex-wrap items-center gap-2"
              >
                <div className="text-sm font-mono">{p.data_type}</div>
                <label className="text-sm">Max Days</label>
                <input
                  className="border rounded px-2 py-1 w-24"
                  defaultValue={p.max_age_days}
                  onBlur={(e) => (p.max_age_days = Number(e.target.value))}
                />
                <label className="text-sm">Archive before delete</label>
                <input
                  type="checkbox"
                  defaultChecked={p.archive_before_delete}
                  onChange={(e) => (p.archive_before_delete = e.target.checked)}
                />
                <label className="text-sm">Encryption required</label>
                <input
                  type="checkbox"
                  defaultChecked={p.encryption_required}
                  onChange={(e) => (p.encryption_required = e.target.checked)}
                />
                <button
                  onClick={() => onRetentionSave(p)}
                  className="ml-auto px-3 py-1 text-xs rounded bg-blue-600 text-white"
                >
                  Save
                </button>
              </div>
        ...
        <div className="mt-6 text-xs text-gray-500">
          Set a valid admin token to use management APIs. API base defaults to
          http://localhost:8000.
        </div>
      </div>
    </div>
