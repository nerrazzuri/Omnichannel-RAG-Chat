  ...
  const uploadFile = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/tenant/upload_file?token=${encodeURIComponent(ADMIN_TEST_TOKEN)}`,
    ...
    <div className="p-4 space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          ...
          <button
            onClick={uploadFile}
            className="inline-flex items-center rounded-md bg-green-600 hover:bg-green-700 text-white text-sm font-medium px-4 py-2"
          >
            Upload File
          </button>
        </div>
