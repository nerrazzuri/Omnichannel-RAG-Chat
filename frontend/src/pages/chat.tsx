    ...
    (async () => {
      try {
        ...
        if (u.ok) {
          ...
          setUsage({
            tokens_used: ju?.["tokens_used"] || 0,
            tokens_quota: ju?.["tokens_quota"] || 0,
            docs_count: ju?.["docs_count"],
          });
        }
      } catch {}
    })();
  ...
  const send = async () => {
    ...
    try {
      ...
      const assistantMsg: Message = {
        ...
        citations: resp.citations?.map(
          (c: any) => c.source_url || c.title || c.source,
        ),
      };

      setMessages((m) => [
        ...m.filter((x) => x.id !== workingId),
        assistantMsg,
      ]);
    } catch (e) {
      console.error(e);
            ...
            <div className="fixed left-0 right-0 bottom-0 bg-gradient-to-t from-white to-white/70 backdrop-blur border-t border-gray-200">
              <div className="max-w-4xl mx-auto px-4 py-4">
                ...
                <div className="mt-1 text-[11px] text-gray-500">
                  Answers are grounded in your uploaded knowledge. Sources are
