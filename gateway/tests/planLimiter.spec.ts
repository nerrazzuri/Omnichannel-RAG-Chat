...
jest.mock('../src/redis/redis', () => ({
  getRedis: () => ({ incr: async () => 9999, expire: async () => {} }),
}));

function mockReq(plan: string, path: string = '/'): any {
  return { path, method: 'GET', claims: { subscription_tier: plan } };
}
function mockRes() {
  const headers: Record<string, string> = {};
  return {
    status: (s: number) => ({ json: (j: any) => ({ status: s, body: j }) }),
